 ...
 * @param {Float64Array} y_data
 * @param {number} num_features
 * @returns {Float64Array}
 */
export function ols(x_data, y_data, num_features) {
    const ptr0 = passArrayF64ToWasm0(x_data, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passArrayF64ToWasm0(y_data, wasm.__wbindgen_malloc);
    const len1 = WASM_VECTOR_LEN;
    const ret = wasm.ols(ptr0, len0, ptr1, len1, num_features);
    if (ret[3]) {
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('ols_wasm_module_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
